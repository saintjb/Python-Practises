...
class Person:
    def __init__(self,  f_name, l_name, age, password, m_name):
        ...
        self.__age = age
        self.__e__email = (self.__f_name[0] + self.__l_name + '@psu.com').lower()
    ...
    def set_first_name(self, f_name):
        if self.string_has_number(f_name):
            print('The first name is incorrect')
        else:
            self.__f_name = f_name.title()
    ...
    def get_email(self):
        return self.__e__email

    def set_email(self, new_email):
        if '@gmail.com' in new_email:
            self.__e__email = new_email
        else:
    ...
    def set_password(self):
        ...
        for tries in range(4):
            if old_password != self.__password:
                ...
                break
            if tries == 3:
                print('\nYou has 5 tries! logging out')
                return exit()
        new_password = getpass('Enter the new password: ')
        while len(new_password) < 6:
            print('Length is to little')
            new_password = getpass('Password should be min 6 characters, enter the new password: ')
        self.__password = new_password

    def administrator_set_password(self, password):
        while len(password) < 6:
            print('Length is to little')
            password = getpass('Password should be min 6 characters, enter the new password: ')
        self.__password = password
...
class Student(Person):
    ...
    def __init__(self, f_name, l_name, age, password, grade, speciality, group_number, m_name=''):
        ...
        self.__id = 'STD' + '_' + self.__grade[0:3] + '_' + self.__speciality[0:3] + '_' + str(self.__group_number) + \
                    '_' +str(self.number_of_students)
        self.__maths = None
    ...
    def set_speciality(self, speciality):
        while speciality.upper() not in ('Python', 'Java', 'PHP'):
            speciality = input('Wrong speciality! Try again: ')
        else:
            self.__speciality = speciality.upper()
            self.set_id()
    ...
    def set_group_number(self, group_number):
        while group_number not in range(1, 30):
            group_number = input('Wrong group number: ')
        else:
            self.__group_number = group_number
            self.set_id()
    ...
    def get_average(self):
        if self.__maths and self.__informatics and self.__electronics:
            ...
            return 'Student does not have all the marks!'

    def define(self):
        if self.get_middle_name():
            print('Student', self.get_first_name(), self.get_last_name(), self.get_middle_name(),
                  'and his ID is:', self.__id)
        else:
            print('Student', self.get_first_name(), self.get_last_name(),
                  'and his ID is:', self.__id)
...
class Professor(Person):
    ...
    def set_speciality(self, speciality):
        while speciality.upper() not in ('MATHS', 'INFORMATICS', 'ELECTRONICS'):
    ...
    def set_teaching_hours(self, teaching_hours):
        while teaching_hours not in range(5, 26):
            input('Teaching hours out of range, enter new: ')
    ...
    def get_salary(self):
        ...
        salary = str(round(basic_salary + (self.get_grade() / 100) * (basic_salary) + self.__teaching_hours * 20), 2) \
                 + '$'
        return salary
...
class Administrator(Person):
    ...
    def __init__(self, f_name, l_name, age, password, grade, speciality, m_name=''):
        ...
        self.__grade = grade
        self.__speciality = speciality.upper()
        self.__id = 'ADM' + str(self.__grade) + '_' + self.__speciality[0:3] + '_' + str(self.number_of_administrators)
    ...
    def define(self):
        if self.get_middle_name():
            print('Administrator', self.get_first_name(), self.get_last_name(), self.get_middle_name(),
                  '- his ID is', self.__id)
        else:
            print('Administrator', self.get_first_name(), self.get_last_name(), '- his ID is:', self.__id)
...
def student_menu(logged_member: Student):
    ...
    while operation not in range(1, 4):
        operation = int(input('Wrong operation, enter an operation again: '))
    if operation == 1:
        logged_member.set_password()
    elif operation == 2:
        print('Maths:', logged_member.get_maths(), '\nInformatics:',
              logged_member.get_informatics(),'\nElectronics', logged_member.get_electronics())
    else:
        print(logged_member.get_average())
...
def professor_menu(logged_member: Professor):
    ...
    if operation == 1:
        ...
        while mark not in range(0, 21):
            mark = int(input('Mark out of range. Please enter a new mark: '))
        {
...
def student_administrator_menu(logged_member: Administrator):
    print('Please select an operation:'
          ...
          '\n\t12)Check students averages'
          '\n\t13)Check students details'
    ...
    if operation == 1:
        ...
        student_number = get_member_number(members[0])
        data = input('Enter the data you want to set: ')
        ...
        student_number = get_member_number(members[0])
        data = input('Please enter the data you want to set: ')
        {
        ...
        }[operation](int(data) if operation == 11 else int(data))
    elif operation == 12:
        student_number = get_member_number(members[0])
...
def professor_administrator_nemu(logged_member: Administrator):
    print('Please select an operation:'
          '\n\t1)Change your password'
    ...
    if operation == 1:
        ...
        if logged_member.get_speciality() == 'ADMINS':
           return admins_menu(logged_member)
            ...
            print('Wrong operation')
    return professor_administrator_nemu(logged_member)
...
def admins_menu(logged_member: Administrator):
    print('Please select what you want to manage:'
          ...
          '\n\t3)Manage strudents')
    manage = int(input('>'))
    while manage not in range(1, 2, 3):
        manage = int(input('Wrong answer! Try again: '))

    def administrator_administrator_menu(logged_member: Administrator):
        print('Please select an operation:'
              '\n\t1)Change your password'
              '\n\t2)Check administrator list'
              '\n\t3)Change administrator first name'
              '\n\t4)Change administratorlast name'
        ...
        if operation == 1:
            ...
            administrator_number = get_member_number(members[1])
            data = input('Please enter the data you want to set: ')
            {
                9: members[2][administrator_number].set_grade,
                10: members[2][administrator_number].set_speciality,
            }[operation](int(data) if operation == 10 else int(data))
        elif operation == 11:
            administrator_number = get_member_number(members[2])
...
def get_members_list(members_list):
    ...
    for member in members:
        print(str(i) + ')' + member.get_first_name(), member.get_last_name(),
              member.get_middle_name())
        i += 1


def get_member_number(members_list):
    print('Enter the number of the %s: '%
          ('student' if members_list == members[0] else 'professor'
           if members_list == members[1] else
           'administrator'))
    member_number = int(input('>')) - 1
    while member_number not in range(0, len(members_list)):
        member_number = int(input('Number out of range! Please '
...
def add_member(members_list):
    ...
    if members_list == members[0]:
        ...
        member.set_group_number(int(input('Enter a group number: ')))
    elif members_list == members[1]:
        member = Professor('a', 'a', 7, 'a', 'a', 'a', 11)
        member.set_teaching_hours(int(input('Enter a teaching hours: ')))
    else:
...
while profession not in (0, 1, 2):
    profession = input('Wrong choice, please enter your profession: ')
logged_member = None
...
if profession == 0:
   student_menu(logged_member)
    ...
    if logged_member.get_speciality() == 'STUDENTS':
        ...
        professor_administrator_nemu(logged_member)
